...
export const TrainingPlanInputSchemaAI = z.object({
  ...
  level: z.coerce.number().min(1).max(12).describe("El nivel de habilidad para el que se diseña el plan (1-12).").optional(),
});
...
export const TrainingPlanInputSchema = z.object({
  ...
  level: z.coerce.number().min(1).max(12).optional(),
});
